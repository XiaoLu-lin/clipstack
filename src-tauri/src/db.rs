...
pub fn upsert_clip(
    ...
    conn.execute(
        "INSERT INTO clips (kind, text, meta, thumb, source, source_icon, source_bundle, pinned, hash, created_at)
         VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, 0, ?8, ?9)
         ON CONFLICT(hash) DO UPDATE SET created_at = excluded.created_at",
        params![kind, text, meta, thumb, source, source_icon, source_bundle, hash, now],
    )?;
    Ok(())
}
